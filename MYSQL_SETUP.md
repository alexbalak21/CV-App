...
`config.py`'s `_build_database_uri()` builds the SQLAlchemy connection
string from these (always via PyMySQL, always `utf8mb4`). If you'd rather
use one connection string instead, `DATABASE_URL` — e.g.
`mysql+pymysql://root:@localhost/cv_app?charset=utf8mb4` — takes priority
over the component variables if set.
